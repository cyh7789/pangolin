...
建議流程：
1. （5s）Slack 收到 Sola Workflow 自動通知
2. （10s）Terminal 跑 `pangolin sola-scan --deep --notify`
3. （15s）Terminal 顯示掃描進度 + LLM 分析
4. （10s）打開 HTML 報告（深色主題，severity badge）
5. （10s）展示 PoC 下載按鈕 + PoC 程式碼
6. （5s）Slack 收到深度分析 + zip
7. （5s）結尾 tagline
...
- 長描述：問題 → 方案 → 技術 → 差異化
- 封面圖
- 程式碼連結（GitHub repo，需建 public repo）

### 🔲 Git 整理

- 目前 git init 完成，尚未 first commit
- `.env` 已在 `.gitignore`
- 建 GitHub public repo 前要確認不洩漏 Bounty 私有模式
